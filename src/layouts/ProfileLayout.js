...
function ProfileLayout({ liveDerham }) {
  ...
  return (
    <>
      <header className="grid-flow-row bg-white shadow-md backdrop-blur sticky top-0 max-w-7xl mx-auto z-2 user-select-none rounded-b-xl">
        <div className="flex items-center justify-between mx-auto max-w-6xl w-full pt-3 pb-2 px-6 xl:px-1">
          ...
          <div className="flex flex-row gap-2 w-full justify-end md:justify-between items-center mr-5">
            ...
            <Link
              to="/cart"
              className="btn max-md:!min-h-10 max-md:!h-10 text-[#203d3f] !bg-transparent border border-[#203d3f] font-extrabold hover:scale-[0.98] p-2 md:px-3 rounded"
            >
              <span>
                <svg
      ...
      <div className="min-h-[calc(100vh-7.75rem)] md:min-h-[calc(100vh-9rem)] flex flex-col md:flex-row w-full max-w-screen-lg md:gap-4 lg:gap-5 md:px-3 mx-auto">
        <div
          className={`${
            pathname.includes("/ticket/") && "max-md:hidden"
          } w-full md:w-[40%] lg:w-[35%] pt-5 md:pt-[84px] max-md:px-3 max-md:sticky max-md:top-[6.25rem] max-md:bg-[#fafafa]/80 backdrop-blur `}
        >
          <ul className="menu md:menu-vertical menu-horizontal bg-gray-300/10 backdrop-blur flex-nowrap overflow-scroll max-md:p-1 gap-1 md:gap-2 w-full rounded-lg shadow">
            {[
              { title: "حساب کاربری", path: "/profile" },
              { title: "سفارشات", path: "/profile/orders" },
              { title: "پشتیبانی", path: "/profile/tickets" },
            ].map((i, idx) => (
              <li key={idx}>
                <Link
                  to={i.path}
                  className={` ${
                    pathname == i.path
                      ? "!bg-[#00796a]/20"
                      : "hover:!bg-[#00796a]/5"
                  } !text-[#203d3f] max-sm:px-2 max-md:px-3`}
                >
                  {i.title}
                </Link>
              </li>
            ))}
            <li className="md:border-t-2 md:pt-2 md:mt-1 max-md:mr-auto mt-auto">
              <Link
                to="/logout"
                className="hover:!bg-red-500/20 hover:!text-red-700 !text-[#203d3f] max-sm:px-2 max-md:px-3"
              >
                خروج از حساب
              </Link>
            </li>
...
export default connect(mapStateToProps, { getLiveDerham })(ProfileLayout);
